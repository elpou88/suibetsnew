...
- **Free Sports API**: Provides data for Basketball, Baseball, Ice Hockey, MMA, American Football, AFL, Formula 1, Handball, Rugby, Volleyball (upcoming only, no live betting for free sports). Free tier limited to 3-day lookahead window (today ± 1 day); `daysAhead: 3` for all sports. Plan errors treated as rate limits to stop early. Data fetched daily at 6 AM UTC, results at 11 PM UTC. Admin force-refresh: POST `/api/admin/free-sports/refresh`. NFL and Tennis API hostnames don't exist — removed from config. Hardcoded scheduled events for Boxing, F1, MMA, MotoGP, Tennis supplement the API for dates beyond the free tier window.
- **MotoGP (sportId 19)**: Full 2026 22-race calendar with 22 real riders and teams. Generated like F1 — rating-based odds with 120% overround, shows next 3 upcoming GPs. Uses RaceEventCard display with rider names and team info. Marc Márquez (Ducati Lenovo) is the defending champion favorite.
- **Boxing (sportId 17)**: Hardcoded real upcoming fights with accurate odds sourced from sportsbooks. Includes title fights (IBF, WBC, WBA, WBO), PPV events, and undercards. Fighter records shown. Events auto-filter to only show future fights.
- **Tennis (sportId 3)**: Hardcoded real upcoming matches from major tournaments — Indian Wells, Miami Open, Monte-Carlo Masters, French Open, Wimbledon, US Open. Real player matchups (Alcaraz, Sinner, Djokovic, Zverev, etc.) with sportsbook-accurate odds. Shows up to 8 upcoming matches.
